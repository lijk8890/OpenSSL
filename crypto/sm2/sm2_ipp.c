...
int ipp_sm2_derive_pubkey(unsigned char prvkey[32], unsigned char pubkey[65])
{
    IppStatus status;
    IppsECCPState *eccp = newSM2_ECCP(SM2_BITS);
    IppsBigNumState *x = newSM2_BN(SM2_INTS, sm2GX);
    ...
    ippsSetOctString_BN(prvkey, 32, k);                 // 设置私钥

    ippsECCPSetPoint(x, y, p, eccp);                    // 设置G点
    status = ippsECCPMulPointScalar(p, k, pk, eccp);    // 点乘公钥
    if(status != ippStsNoErr)
    {
        fprintf(stderr, "%s:%d - %s\n", __FILE__, __LINE__, ippGetStatusString(status));
        goto ErrP;
    }
    ippsECCPGetPoint(x, y, pk, eccp);                   // 获取公钥
